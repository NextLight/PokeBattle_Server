    ...
    class Battle
    {
        ...
        private void ExecuteMove(int p, int? m)
        {
            ...
            if ((move.Accuracy ?? 100) >= rand.Next(101))
            {
                ...
                {
                    bool critical = rand.Next(16 / (1 + move.CriticalRate)) == 0;
                    int damage = CalculateDamage(pokemons[p].Level, move.Power.Value,
                        move.DamageClass == DamageClass.Physical ? pokemons[p].InBattle.Stats.Attack : pokemons[p].InBattle.Stats.SpecialAttack, 
                        move.DamageClass == DamageClass.Physical ? pokemons[p ^ 1].InBattle.Stats.Defense : pokemons[p ^ 1].InBattle.Stats.SpecialDefense);
                    ...
                    pokemons[p ^ 1].InBattle.Stats.Hp -= damage; // eh xor, beware // TODO: change to something saner
                }
            }
